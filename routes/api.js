...
module.exports = function (passport) {
    ...
    router.route('/users/:id')
        ...
        .delete(async (req, res) => {
            ...
            let mails = await Mail.getSubjectAndIdByLogin(req.user.login, req.user.id);
    ...
    router.route('/mails')
        ...
        .post(upload.array('file'), async (req, res) => {
            ...
            let sender = await User.getByLogin(mail.sender);
            let receiver = await User.getByLogin(mail.receiver);
            ...
            if (files) {
                ...
                try {
                    for (let i = 0; i < files.length; i++) {
                        let fileData = fs.readFileSync(files[i].path);
    ...
    router.route('/mails/:id')
        ...
        .put(async (req, res) => {
            ...
            let mail = await Mail.getById(id);
    ...
    router.get('/chats/:id/get_pages', async (req, res) => {  //get amount info
        ...
        const entitiesPerPage = 4;
        try {
            let chat = await Chat.getById(id);
